...
export function useGameProgram() {
    ...
    const initializePlayer = useCallback(async (): Promise<string> => {
        ...
        try {
            const tx = await program.methods
                .initializePlayer()
                .accounts({
                    city: getPlayerProfilePda(wallet.publicKey, program.programId),
        ...
        try {
            ...
            const tx = await program.methods
                ...
                .accounts({
                    ...
                    cardMintState: getCardMintStatePda(mint, program.programId),
                    authority: wallet.publicKey,
                    systemProgram: SystemProgram.programId,
    ...
    // Set Deck (Base Layer)
    const setDeck = useCallback(async (newDeck: number[]): Promise<string> => {
        if (!program || !wallet.publicKey) throw new Error("Wallet not connected");
        setIsLoading(true);
        try {
            const tx = await program.methods
    ...
    const resolveGame = useCallback(async (game: PublicKey, battle: PublicKey, playerOne: PublicKey, playerTwo: PublicKey): Promise<string> => {
        if (!program || !wallet.publicKey) throw new Error("Wallet not connected");
        setIsLoading(true);
        try {
            const tx = await program.methods
                ...
                .accounts({
                    ...
                    playerOne: playerOne,
                    playerTwo: playerTwo,
                    // systemProgram not explicitly required by resolve_game unless it does closes/transfers? 
                    // IDL for resolve_game only lists: game, battle, player_one, player_two. 
                    // But if it modifies accounts, it might need system program if not implied.
                    // IDL verification: resolve_game accounts: game, battle, player_one, player_two. No system_program.
                } as any)
                .rpc();
            return tx;
        } catch (err: any) {
